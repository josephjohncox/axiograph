# Explanation

Explanation docs describe why the current system works the way it does. They
should not be used as a place to preserve old protocol or prototype history.

## Current Architecture

- `docs/explanation/BOOK.md`
- `docs/explanation/SYSTEM_OVERVIEW.md`
- `docs/explanation/ARCHITECTURE.md`
- `docs/explanation/VERIFICATION_AND_GUARDRAILS.md`
- `docs/explanation/COMPONENT_VALUE.md`
- `docs/explanation/CONSTRAINT_SEMANTICS.md`
- `docs/explanation/PATHDB_DESIGN.md`
- `docs/explanation/SEMANTIC_WEB_INTEROP.md`
- `docs/explanation/TYPED_ONTOLOGY_ENGINEERING.md`
- `docs/explanation/DDD_CONTEXT_WRAPPERS.md`

## Theory And Design Direction

- `docs/explanation/MATHEMATICAL_FOUNDATIONS.md`
- `docs/explanation/HOTT_FOR_KNOWLEDGE_GRAPHS.md`
- `docs/explanation/PATH_VERIFICATION.md`
- `docs/explanation/TYPE_THEORY_DESIGN.md`
- `docs/explanation/RUST_DEPENDENT_TYPES.md`
- `docs/explanation/TOPOS_THEORY.md`
- `docs/explanation/KNOWLEDGE_GENERATION_AND_LEARNING.md`

## Execution And Extension Layers

- `docs/explanation/DISTRIBUTED_PATHDB.md`
- `docs/explanation/RECONCILIATION.md`
- `docs/explanation/QUERY_SPEC_CONVERGENCE.md`
- `docs/explanation/JEPA_INTEGRATION.md`
- `docs/explanation/OBJECTIVE_DRIVEN_AI.md`
- `docs/explanation/SELF_SUPERVISED_LEARNING.md`
- `docs/explanation/LLM_KG_SYNC.md`

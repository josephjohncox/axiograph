...
| Runtime conversation sync | At most 4,096 turns and 8 MiB across content plus metadata; strict config with unknown fields rejected; finite confidence threshold; extraction batch in `1..=10,000`; extracted facts reject during streaming when the configured batch is exhausted. |

...
# Exact-lock RustSec audit with documented RDF/XML preflight exceptions
make verify-rustsec
...
| `make verify-rustsec` | Exact Rust lockfile advisory gate | Requires cargo-audit 0.22.2 and audits both workspace and isolated fuzz lockfiles. It rejects every RustSec vulnerability except the two quick-xml 0.37 findings reached only after the exact RDF/XML bytes pass the patched quick-xml 0.41 attribute, namespace, depth, event, and input bounds documented in `SECURITY_BOUNDARIES.md`. |

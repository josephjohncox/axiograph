...
# - `neigh ... --out ...` (REPL-driven JSON graph export with plane filters)
#
# Run from repo root:
...
"$AXIOGRAPH" repl --continue-on-error \
  ...
  --cmd "neigh Alice --plane both --hops 3 --format json --out $OUT_DIR/family_alice.json --max_nodes 260" \
  ...
  --cmd "neigh acme.svc0.v1.Service0 --plane data --hops 2 --format json --out $OUT_DIR/proto_api_service0.json --max_nodes 340" \
  --cmd "neigh doc_proto_api_0 --plane data --hops 2 --format json --out $OUT_DIR/proto_api_doc0.json --max_nodes 380"

echo ""
echo "Done."
echo "Outputs:"
echo "  $OUT_DIR/family_alice.json"
